...
impl DownlinkManager {
    ...
    async fn process_downlink(msg: Msg) -> DeviceResult {
        ...
        match down.device {
            DeviceType::LoRaNode => match LoRaNodeManager::get_node_by_eui(down.eui).await? {
                Some(s) => {
                    let data =
                    ...
                    tokio::spawn(async move {
                        if let Err(e) = s
                            .dispatch_task_now(DownloadData::new_data_with_id_and_forward(
                                data,
                                1,
                                ClientId::next(),
                                down.port,
                ...
                None => return Err(DeviceError::Device("not found device".to_string())),
            },
            DeviceType::Snap => {
                let mut conn = RedisClient::get_client().get_multiplexed_conn().await?;
                let snap = SnapDeviceInfo::load(down.eui, &mut conn)
                    .await?
                    .ok_or_else(|| DeviceError::Device("Error loading snap".to_string()))?;
